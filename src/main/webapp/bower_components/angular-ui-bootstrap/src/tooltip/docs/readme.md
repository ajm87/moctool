...
* `tooltip-placement`
  ...
  Passing in 'auto' separated by a space before the placement will enable auto positioning, e.g: "auto bottom-xPos". The tooltip will attempt to position where it fits in the closest scrollable ancestor. Accepts:

   * `top` - tooltip on top, horizontally centered on host element.
   * `top-xPos` - tooltip on top, xPos edge aligned with host element xPos edge.
   ...
   * `bottom-xPos` - tooltip on bottom, xPos edge aligned with host element xPos edge.
   * `bottom-right` - tooltip on bottom, right edge aligned with host element right edge.
   * `xPos` - tooltip on xPos, vertically centered on host element.
   * `xPos-top` - tooltip on xPos, top edge aligned with host element top edge.
   * `xPos-bottom` - tooltip on xPos, bottom edge aligned with host element bottom edge.
