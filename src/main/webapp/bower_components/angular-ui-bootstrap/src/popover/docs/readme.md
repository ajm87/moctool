...
* `popover-placement`
  ...
  Passing in 'auto' separated by a space before the placement will enable auto positioning, e.g: "auto bottom-xPos". The popover will attempt to position where it fits in the closest scrollable ancestor. Accepts:

   * `top` - popover on top, horizontally centered on host element.
   * `top-xPos` - popover on top, xPos edge aligned with host element xPos edge.
   ...
   * `bottom-xPos` - popover on bottom, xPos edge aligned with host element xPos edge.
   * `bottom-right` - popover on bottom, right edge aligned with host element right edge.
   * `xPos` - popover on xPos, vertically centered on host element.
   * `xPos-top` - popover on xPos, top edge aligned with host element top edge.
   * `xPos-bottom` - popover on xPos, bottom edge aligned with host element bottom edge.
